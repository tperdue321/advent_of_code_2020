...
# so you would need to wait 944 - 939 = 5 minutes before it departs. Multiplying the bus ID by the number of minutes
...
# Those bus IDs can depart at other times, and other bus IDs can depart at those times. For example, in the list above,
# because bus ID 19 must depart seven minutes after the timestamp at which bus ID 7 departs, bus ID 7 will always also be
...
pp(begin
  _timestamp, bus_lines = IO.read('input.txt').split("\n")
  bus_lines = bus_lines.split(',')
    .each_with_index
    .reject { |bus| bus[0] == 'x' }
    .map { |bus_id, index| [bus_id.to_i, -index.to_i % bus_id.to_i] }
    .map { |bus| [bus[0].to_i, bus[1].to_i] }

  lcd = bus_lines.map { _1[0] }.reduce(:*)
  foobar = bus_lines.map do |bus_info|
    bus_id, rem = bus_info
    puts rem
    inv = inv_finder(lcd / bus_id, bus_id)
    [rem, lcd / bus_id, inv].reduce(:*)
  end
